...
main() {
  ...
  invert_select_window_fg=$(get_tmux_option "@dracula-invert-select-window-fg" false)
  ...
  for plugin in "${plugins[@]}"; do
    ...
    elif [ $plugin = "kitty" ]; then
      IFS=' ' read -r -a colors <<<$(get_tmux_option "@dracula-kitty-colors" "green dark_gray")
      script="#($current_dir/kitty.sh)"
  ...
  # Window option
  if $show_powerline; then
    # Define your optional flag (this could be passed from the outside or set in your config)
    if $invert_select_window_fg; then
