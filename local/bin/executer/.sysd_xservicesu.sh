...
services=$(systemctl --user list-units --type=service --no-pager --no-legend |
  awk '$4 == "running" {$2=$3=""; print $0}' | while read -r service; do
  service_name="$(echo "$service" | awk '{print $1}')"
  description="$(systemctl --user show -p Description "$service_name" | sed 's/Description=//')"
...
# Show the services in a Rofi menu
selected=$(echo "$formatted_services" | rofi -dmenu -p "Select a service:" \
