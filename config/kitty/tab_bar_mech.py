...
class WeirdTabBar(TabBar):
    ...
    def update(self, data: Sequence[TabBarData]) -> None:
        right_status = " " + datetime.datetime.now().strftime(
            "%a, %b %-d %H:%M",
        )
        left_logo = _get_logo()
        if len(left_logo) > 0:
        ...
        match opts.tab_bar_align:
            case "left":
                self.inner.align: Callable[[], None] = partial(  # noqa # pyright: ignore
...
def _get_logo() -> str:
    ...
    match _os:
        case "Darwin":
            return "󰀵"
        case "GNU/Linux":
            distro_os = os.popen('hostnamectl | grep "Operating System"')
            distro = distro_os.read().strip()
            match distro:
