...
STATE_FILE="$XDG_RUNTIME_DIR/musicterm_state.json"
WINDOW_TITLE="MusicTerm"
...
if [ -z "$WIN_ID" ]; then
  echo "[ERROR] Failed to relaunch MusicTerm."
  if [[ "${BASH_SOURCE[0]}" != "$0" ]]; then
    return 1
  else
    exit 1
...
echo "[INFO] MusicTerm restored to previous state."
